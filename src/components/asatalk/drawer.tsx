...
import { Bookmark, LogOut, Megaphone, Moon, Phone, Plus, Settings, UserPlus, Users, Video } from "lucide-react";
...
export function TalkDrawer({ onLogout, onAddAccount, onSwitch }: { onLogout: () => void; onAddAccount: () => void; onSwitch: (userId: string) => void }) {
  const t = useT();
  const { dir } = useLocale();
  ...
  return (
    <AnimatePresence>
      {drawerOpen && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] bg-black/40 backdrop-blur-sm" onClick={() => setDrawer(false)} />
          <motion.aside
          ...
          >
            ...
            <div className="tg-scroll flex-1 p-2">
              ...
              {asameetUrl && (
                <a href={asameetUrl} target="_blank" rel="noopener noreferrer" className="tg-row text-[14px] font-medium">
