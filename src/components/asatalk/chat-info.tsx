...
export function ChatInfo({
  ...
  const [mediaTab, setMediaTab] = useState<"media" | "files" | "voice">(
    "media",
  );
  ...
  const isAdmin =
    chat.myRole === "owner" || chat.myRole === "admin" || me.role === "admin";
  ...
  const media = useMemo(
    () =>
      (msgsQ.data?.messages ?? []).filter(
        (m) => m.type === "image" || m.type === "video",
      ),
    [msgsQ.data],
  );
  const files = useMemo(
    () => (msgsQ.data?.messages ?? []).filter((m) => m.type === "file"),
    [msgsQ.data],
  );
  const voices = useMemo(
    () =>
      (msgsQ.data?.messages ?? []).filter(
        (m) => m.type === "voice" || m.type === "video_note",
      ),
    [msgsQ.data],
  );
  ...
  const roleOf = (id: string) =>
    id === chat.createdBy || (chat.adminIds ?? []).includes(id)
      ? (chat.adminIds ?? []).includes(id) && id !== chat.createdBy
        ? "admin"
        : "owner"
      : "member";
    ...
    <div className="flex h-full flex-col">
      <GHeader
        title={
          chat.type === "private"
            ? t("talk.info.title")
            : chat.type === "group"
              ? t("talk.info.groupInfo")
              : t("talk.info.channelInfo")
        }
        onBack={onClose}
        right={
      ...
      <div className="tg-scroll flex-1 pb-6">
        ...
        <div className="relative flex flex-col items-center gap-2 px-4 pt-6 pb-4 text-center">
          <div
            className="absolute inset-x-0 top-0 -z-10 h-40 opacity-30"
            style={{
              background:
                "radial-gradient(60% 80% at 50% 0%, var(--talk-tint-2), transparent)",
            }}
          />
          <TalkAvatar
            name={title}
          ...
          <p
            className={cn("text-xs", peer?.isOnline ? "text-talk" : "tg-muted")}
          >
            {chat.type === "private"
              ? saved
        ...
        <div className="px-3">
          ...
          <GSection>
            {peer && !saved && (
              <GItem
                ...
                label={<span dir="ltr">@{peer.username}</span>}
                value={t("talk.info.username")}
                onClick={() => {
                  void navigator.clipboard.writeText(`@${peer.username}`);
            ...
            {peer && !saved && (
              <GItem
                ...
                label={
                  peer.bio || (
                    <span className="tg-muted">{t("talk.info.noBio")}</span>
                  )
                }
                value={t("talk.info.bio")}
              />
            )}
            {chat.type !== "private" && chat.description && (
              <GItem
                icon={<FileText className="size-4" />}
                color="linear-gradient(135deg,#f59e0b,#d97706)"
                label={
            ...
            {chat.type !== "private" && chat.username && (
              <GItem
                icon={<AtSign className="size-4" />}
                label={<span dir="ltr">@{chat.username}</span>}
                value={t("talk.info.publicLink")}
                onClick={() => {
                  ...
                  isAdmin ? (
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        void run(() =>
                          talkApi.updateChat(chat.id, { resetInvite: true }),
                        );
                      }}
                      aria-label={t("talk.info.resetLink")}
                    >
            ...
            <GItem
              ...
              right={
                <GSwitch
                  on={!chat.isMuted}
                  onChange={(v) =>
                    void run(() => talkApi.chatPrefs(chat.id, { muted: !v }))
                  }
                />
              }
            />
              ...
              {mediaTab === "files" &&
                (files.length ? (
                  files.map((m) => (
                    <a
                      key={m.id}
                      href={mediaUrl(m.mediaId!)}
                      download={m.meta?.fileName}
                      className="tg-row"
                    >
                      <span className="tg-play !h-9 !w-9 !rounded-xl">
                        <FileText className="size-4" />
                      </span>
                      <span className="truncate text-sm">
                        {m.meta?.fileName ?? t("talk.msg.file")}
                      </span>
                    </a>
                  ))
                ) : (
                ...
                (voices.length ? (
                  voices.map((m) => (
                    <div key={m.id} className="tg-row cursor-default">
                      <span className="tg-play !h-9 !w-9">
                        {m.type === "voice" ? (
                      ...
                      <span className="text-sm">
                        {users.get(m.senderId)?.displayName}
                      </span>
                      <span className="tg-muted ms-auto text-xs">
                        {toLocaleDigits(m.meta?.duration ?? 0, locale)}s
                      </span>
                    </div>
                  ))
                ) : (
          ...
          {chat.type !== "private" && (
            <GSection
              title={`${t("talk.info.members")} · ${toLocaleDigits(members.length, locale)}`}
            >
              {(chat.type === "group" || isAdmin) && (
                <GItem
                  icon={<UserPlus className="size-4" />}
                  label={t("talk.info.addMembers")}
                  ...
                  <div key={u.id} className="tg-item cursor-default">
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      >
                        {u.isOnline
                          ? t("common.online")
                          : lastSeenLabel(u, t, locale)}
                      </span>
                    </span>
                    {role !== "member" && (
                      <span className="tg-muted text-caption inline-flex items-center gap-1">
                        {role === "owner" ? (
                        ...
                        <GMenuContent align="end">
                          ...
                          {(isOwner || role === "member") && (
                            <GMenuItem
                              ...
                              onSelect={() =>
                                void run(() =>
                                  talkApi.chatMembers(chat.id, "remove", u.id),
                                )
                              }
                            >
          ...
          <GSection>
            {peerId && !saved && (
              <GItem
                ...
                onClick={() => void toggleBlock(peerId)}
              />
            )}
            {chat.type !== "private" && (
              <GItem
            ...
            {(chat.type === "private" || isOwner) && (
              <GItem
                ...
                label={
                  chat.type === "private"
                    ? t("talk.info.deleteChat")
                    : chat.type === "group"
                      ? t("talk.info.deleteGroup")
                      : t("talk.info.deleteChannel")
                }
                danger
                onClick={() =>
                  setConfirm({
      ...
      {editing && (
        <EditChatDialog chat={chat} onClose={() => setEditing(false)} />
      )}
      {adding && (
        <AddMembersDialog
          chat={chat}
          ...
          onAdd={async (ids) => {
            for (const id of ids)
              await talkApi.chatMembers(chat.id, "add", id).catch(showError);
            await refreshChats();
            setAdding(false);
          }}
        />
...
function Empty() {
  const t = useT();
  return (
    <p className="tg-muted p-4 text-center text-xs">{t("talk.info.noMedia")}</p>
  );
}
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="talk max-w-md !rounded-3xl">
        ...
        <textarea
          ...
          placeholder={t("talk.info.description")}
          className="tg-input min-h-20"
          maxLength={255}
        />
        <div>
          <div className="relative">
            <span className="tg-muted absolute start-3 top-1/2 -translate-y-1/2 text-sm">
              @
            </span>
            <input
              value={username}
              onChange={(e) => setUsername(e.target.value.toLowerCase())}
              placeholder={t("talk.info.publicLink")}
              className="tg-input ps-8"
              dir="ltr"
              maxLength={32}
            />
          </div>
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="talk max-w-md !rounded-3xl">
        ...
        >
          <UserPlus className="size-4" /> {t("talk.info.addMembers")} (
          {sel.length})
        </GBtn>
      </DialogContent>
    </Dialog>
