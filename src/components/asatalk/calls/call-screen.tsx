...
export function CallScreen({
  ...
  const hasRemoteVideo =
    !!state.remote?.getVideoTracks().some((v) => v.readyState === "live") &&
    (state.peerState.camera || state.peerState.screen);
  const hasLocalVideo = !!state.local?.getVideoTracks().some((v) => v.enabled);

  useEffect(() => {
    if (state.phase !== "connected") return;
    const id = window.setInterval(
      () => setSeconds(state.session.duration),
      1000,
    );
      ...
      case "ringing":
        return incoming
          ? state.call.type === "video"
            ? t("talk.calls.incomingVideo")
            : t("talk.calls.incomingAudio")
          : t("talk.calls.ringing");
      case "connecting":
        ...
        onPointerDown={(e) => {
          ...
          const move = (ev: PointerEvent) => {
            const nx = Math.min(
              ...
              Math.max(0, ev.clientX - start.x),
            );
            const ny = Math.min(
              window.innerHeight - 200,
              Math.max(0, ev.clientY - start.y),
            );
      ...
      >
        <div className="tg-call-bg relative h-full w-full">
          ...
          <div className="absolute inset-x-0 bottom-2 flex justify-center gap-2">
            <button
              className="tg-call-btn !h-9 !w-9"
              onClick={() => onMinimize(false)}
              aria-label={t("talk.calls.expand")}
            >
              <Maximize2 className="size-4" />
            </button>
            <button
              className="tg-call-btn tg-call-end !h-9 !w-9"
              onClick={onHangup}
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0 }}
        className={cn(
          "talk tg-call-bg z-call fixed inset-0 flex flex-col overflow-hidden text-white",
          state.phase === "ended" && "saturate-50",
        )}
        role="dialog"
        aria-label={
          state.call.type === "video"
      ...
      >
        ...
        {hasRemoteVideo && (
          <div className="absolute inset-0">
            <VideoEl
              stream={state.remote}
              className={
                state.peerState.screen ? "bg-black object-contain" : undefined
              }
            />
        ...
        {/* Top bar */}
        <div className="tg-safe-top z-chrome relative flex items-start justify-between px-4 pt-3">
          {state.phase === "connected" ? (
            <div className="min-w-0">
              <h2 className="truncate text-[20px] font-black drop-shadow">
          ...
          <span className="text-caption inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 font-semibold backdrop-blur">
            <ShieldCheck className="size-3.5" /> {t("talk.calls.encrypted")}
          </span>
        </div>

        {/* Center */}
        <div className="z-chrome relative flex min-h-0 flex-1 flex-col items-center justify-center gap-4 overflow-hidden px-6">
          {state.phase === "ended" ? (
            <motion.div
            ...
            >
              <TalkAvatar
                ...
                src={state.peer.avatar}
                size="xl"
                className="!size-[84px] !shadow-2xl grayscale-[0.5]"
              />
              <h2 className="text-heading font-black drop-shadow">{status}</h2>
              {state.reason === "failed" && (
                <p className="text-preview max-w-xs leading-6 text-white/70">
                  {t("talk.calls.failedHint")}
                </p>
              )}
              ...
              <Mascot
                pose={
                  state.reason === "declined" || state.reason === "failed"
                    ? "sad"
                    : "wave"
                }
                size={110}
            ...
            <>
              {!hasRemoteVideo && (
                ...
                >
                  ...
                  <TalkAvatar
                    ...
                    src={state.peer.avatar}
                    size="xxl"
                    className="!size-[150px] !shadow-2xl"
                  />
                </motion.div>
              )}
              {state.phase !== "connected" && (
                <div className="text-center">
                  <h2 className="text-[29px] font-black drop-shadow">
                    {state.peer.displayName}
                  </h2>
                  <p className="text-item mt-1 text-white/85">
                    {status}
                    {state.phase === "ringing" ? "…" : ""}
                  </p>
                </div>
              ...
              {state.phase === "connected" && state.peerState.muted && (
                <span className="absolute bottom-6 inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1.5 text-[12px] backdrop-blur">
                  ...
                  {t("talk.conv.peerMuted").replace(
                    "{name}",
                    state.peer.displayName,
                  )}
                </span>
              )}
              {state.phase === "connected" && state.peerState.screen && (
                <p className="absolute top-2 text-xs text-white/70">
                  {t("talk.calls.peerSharing")}
                </p>
              )}
              {state.phase === "ringing" && !hasRemoteVideo && (
                <div className="opacity-90">
                  <Mascot
                    pose={state.call.type === "video" ? "video" : "phone"}
                    size={140}
                  />
                </div>
              )}
        ...
        {hasLocalVideo && state.phase !== "ended" && (
          <motion.div
            drag
            dragMomentum={false}
            className="z-panel absolute start-[18px] top-24 h-40 w-28 overflow-hidden rounded-2xl border border-white/30 shadow-2xl"
          >
        ...
        {/* Controls */}
        <div className="tg-safe-bottom z-chrome relative shrink-0 px-6 pb-8">
          {incoming ? (
            <div className="flex flex-col items-center gap-6">
              <button
                type="button"
                className="tg-btn text-body !rounded-full !bg-white/15 px-5 !text-white"
                onClick={onMessage}
              >
                <MessageCircle className="size-4" />{" "}
                {t("talk.conv.replyWithMessage")}
              </button>
            ...
            <div className="mx-auto flex max-w-xs gap-3">
              <button
                type="button"
                className="tg-btn tg-btn-primary h-12 flex-1 !rounded-full text-sm"
                onClick={onCallAgain}
              >
                {state.call.type === "video" ? (
                ...
                {t("talk.conv.callAgain")}
              </button>
              <button
                type="button"
                className="tg-btn h-12 flex-1 !rounded-full !bg-white/15 text-sm !text-white"
                onClick={onMessage}
              >
            ...
            <div className="mx-auto flex max-w-md items-center justify-center gap-3">
              <Ctl
                ...
                label={
                  state.muted ? t("talk.calls.unmute") : t("talk.calls.mute")
                }
              >
                {state.muted ? (
              ...
              <Ctl
                on={!state.camera}
                onClick={() => void onCamera(!state.camera)}
                label={
                  state.camera
                    ? t("talk.calls.cameraOff")
                    : t("talk.calls.cameraOn")
                }
              >
                {state.camera ? (
              ...
              <Ctl
                on={state.screen}
                onClick={() => void onScreen(!state.screen)}
                label={
                  state.screen
                    ? t("talk.calls.stopShare")
                    : t("talk.calls.shareScreen")
                }
                hideOnMobile
              >
          ...
          {state.phase !== "ended" && (
            <p className="text-caption mt-4 text-center text-white/60">
              <Camera className="me-1 inline size-3" /> {me.displayName} ·{" "}
              {t("talk.calls.you")}
            </p>
          )}
        </div>
...
function Ctl({
  ...
  return (
    <button
      className={cn(
        "tg-call-btn disabled:opacity-40",
