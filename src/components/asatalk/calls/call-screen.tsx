  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0 }}
        className="talk tg-call-bg fixed inset-0 z-[80] flex flex-col overflow-hidden text-white"
        role="dialog"
      ...
      >
        ...
        {/* Top bar */}
        <div className="tg-safe-top relative z-10 flex items-center justify-between px-4 pt-3">
        ...
        <div className="relative z-10 flex min-h-0 flex-1 flex-col items-center justify-center gap-4 overflow-hidden px-6">
          {!hasRemoteVideo && (
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative">
              ...
              <TalkAvatar name={state.peer.displayName} src={state.peer.avatar} size="xxl" className="!shadow-2xl" />
            </motion.div>
          )}
          <div className={cn("text-center", hasRemoteVideo && "absolute top-16")}>
            <h2 className="text-2xl font-black drop-shadow">{state.peer.displayName}</h2>
            <p className="mt-1 text-sm text-white/85">{status}</p>
            {state.phase === "connected" && state.peerState.muted && <p className="mt-1 text-xs text-white/70">{t("talk.calls.peerMuted")}</p>}
            {state.phase === "connected" && state.peerState.screen && <p className="mt-1 text-xs text-white/70">{t("talk.calls.peerSharing")}</p>}
          </div>
          {state.phase === "ringing" && !hasRemoteVideo && (
            <div className="opacity-90">
              <Mascot pose={state.call.type === "video" ? "video" : "phone"} size={150} />
            </div>
          )}
          {state.phase === "ended" && <Mascot pose={state.reason === "declined" || state.reason === "failed" ? "sad" : "wave"} size={130} />}
        </div>

        {/* Local preview */}
        {hasLocalVideo && state.phase !== "ended" && (
          <motion.div drag dragMomentum={false} className="absolute end-4 top-20 z-20 h-44 w-32 overflow-hidden rounded-2xl border border-white/30 shadow-2xl">
            <VideoEl stream={state.local} muted mirror={!state.screen} />
          </motion.div>
        )}

        {/* Controls */}
        <div className="tg-safe-bottom relative z-10 shrink-0 px-6 pb-6">
          {incoming ? (
            <div className="flex items-center justify-center gap-10">
              <div className="flex flex-col items-center gap-2">
                <button className="tg-call-btn tg-call-end !h-16 !w-16" onClick={onDecline} aria-label={t("talk.calls.decline")}>
                  <PhoneOff className="size-6" />
                </button>
                <span className="text-xs">{t("talk.calls.decline")}</span>
              </div>
              <div className="flex flex-col items-center gap-2">
                <button className="tg-call-btn tg-call-accept tg-ring-shake !h-16 !w-16" onClick={onAccept} aria-label={t("talk.calls.accept")}>
                  {state.call.type === "video" ? <Video className="size-6" /> : <Phone className="size-6" />}
                </button>
            ...
            <div className="mx-auto flex max-w-md flex-wrap items-center justify-center gap-3">
              ...
              {state.camera && (
                <Ctl on={false} onClick={onSwitchCamera} label={t("talk.calls.switchCamera")}>
              ...
              <button className="tg-call-btn tg-call-end !h-16 !w-16" onClick={onHangup} aria-label={t("talk.calls.end")} disabled={state.phase === "ended"}>
                <PhoneOff className="size-6" />
              </button>
            </div>
          )}
...
function Ctl({ on, onClick, label, children, hideOnMobile }: { on: boolean; onClick: () => void; label: string; children: React.ReactNode; hideOnMobile?: boolean }) {
  return (
    <button className={cn("tg-call-btn", hideOnMobile && "hidden sm:inline-flex")} data-on={on} onClick={onClick} aria-label={label} title={label}>
      {children}
    </button>
  );
}
