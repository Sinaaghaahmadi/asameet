...
export function CallsPage({ onBack }: { onBack: () => void }) {
  ...
  const callsQ = useQuery({ queryKey: ["talk", "calls"], queryFn: () => talkApi.calls(), refetchInterval: 15_000 });
  const calls = callsQ.data?.calls ?? [];
  const contacts = useMemo(() => userList.filter((u) => u.id !== me.id && !u.isSuspended && (!q || u.displayName.toLowerCase().includes(q.toLowerCase()))), [userList, me.id, q]);

  const meta = (c: Call) =>
    c.direction === "missed"
      ? { Icon: PhoneMissed, cls: "text-red-500", label: t("talk.calls.missed") }
      : c.direction === "incoming"
        ? { Icon: PhoneIncoming, cls: "text-emerald-500", label: t("talk.calls.incoming") }
        : { Icon: PhoneOutgoing, cls: "text-[var(--talk)]", label: c.status === "declined" ? t("talk.calls.declined") : t("talk.calls.outgoing") };

  return (
    <div className="flex h-full flex-col">
      <GHeader title={t("talk.calls.title")} onBack={onBack} />
      <div className="flex gap-1 px-3 pt-2">
        <button type="button" className="tg-chip" data-active={tab === "recent"} onClick={() => setTab("recent")}>
          {t("talk.calls.recent")}
        </button>
        <button type="button" className="tg-chip" data-active={tab === "contacts"} onClick={() => setTab("contacts")}>
        ...
        {tab === "recent" ? (
          calls.length === 0 ? (
            <div className="flex flex-col items-center gap-2 p-8 text-center">
              ...
              <GBtn variant="primary" size="sm" onClick={() => setTab("contacts")}>
            ...
            {contacts.map((u) => (
              <div key={u.id} className="tg-row cursor-default">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className={cn("block text-xs", u.isOnline ? "text-[var(--talk)]" : "tg-muted")}>{u.isOnline ? t("common.online") : t("common.offline")}</span>
                </span>
                <button type="button" className="tg-btn tg-icon" onClick={() => void startCall(u, "audio")} aria-label={t("talk.calls.audioCall")}>
                  <Phone className="size-4" />
                </button>
                <button type="button" className="tg-btn tg-icon" onClick={() => void startCall(u, "video")} aria-label={t("talk.calls.videoCall")}>
