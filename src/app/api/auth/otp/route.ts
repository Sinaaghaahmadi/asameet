...
export async function POST(req: NextRequest) {
  try {
      ...
      if (webhook) {
        const res = await fetch(webhook, {
          ...
          headers: {
            ...
            ...(process.env.ASATALK_OTP_WEBHOOK_TOKEN
              ? {
                  Authorization: `Bearer ${process.env.ASATALK_OTP_WEBHOOK_TOKEN}`,
                }
              : {}),
          },
          body: JSON.stringify({
    ...
    const data = await rpc<{ user: User; token: string; isNew: boolean }>(
      "api_otp_verify",
      {
        p_identifier: identifier,
        p_code: body.code,
    ...
    const res = attachSession(
      NextResponse.json({ user: data.user, isNew: data.isNew }),
      data.token,
    );
